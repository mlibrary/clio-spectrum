...
describe 'Google Analytics' do
  ...
  context 'catalog search results page' do
    context 'Selected Items dropdown' do
      ...
      context 'Send to Email' do
        ...
        context 'data-ga-category' do
          it{expect(link.find(:xpath, 'ancestor-or-self::*[@data-ga-category]')['data-ga-category']).to eq('Catalog Results List')}
        end
        context 'data-ga-action' do
          it{expect(link.find(:xpath, 'ancestor-or-self::*[@data-ga-action]')['data-ga-action']).to eq('Selected Items')}
        end
        context 'data-ga-label' do
          it{expect(link.text).to eq('Send to Email')}
        end
      end
      ...
      context 'Export to EndNote' do
        ...
        context 'data-ga-label' do
          it{expect(link.text).to eq('Export to EndNote')}
        end
      end
      ...
      context 'Add to My Saved List' do
        ...
        context 'data-ga-label' do
          it{expect(link.text).to eq('Add to My Saved List')}
        end
      end
      ...
      context 'Select All' do
        ...
        context 'data-ga-label' do
          it{expect(link.text).to eq('Select All Items')}
        end
      end
      ...
      context 'Clear All' do
        ...
        context 'data-ga-label' do
          it{expect(link.text).to eq('Clear All Items')}
        end
      end
    end
    ...
    context 'Display Options dropdown' do
      ...
      context 'currently set to 50 items per page' do
        context 'link to change from 50 to 25 items per page' do
          ...
          context 'data-ga-action' do
            it{expect(link.find(:xpath, 'ancestor-or-self::*[@data-ga-action]')['data-ga-action']).to eq('Display Options')}
          end
          context 'data-ga-label' do
            it{expect(link.text).to eq('25 per page')}
          end
        end
        ...
        context 'link to change from 50 to 100 items per page' do
          ...
          context 'data-ga-label' do
            it{expect(link.text).to eq('100 per page')}
          end
      ...
      context 'in Standard View' do
        ...
        context 'link to change to Compact View' do
          ...
          context 'data-ga-label' do
            it{expect(link.text).to eq('Compact View')}
          end
        end
      ...
      ['Relevance', 'Acquired Earliest', 'Acquired Latest', 'Published Earliest', 'Published Latest',
       'Author A-Z', 'Author Z-A', 'Title A-Z', 'Title Z-A'].each do |option|
        context "#{option}" do
          ...
          context 'data-ga-action' do
            it{expect(link.find(:xpath, 'ancestor-or-self::*[@data-ga-action]')['data-ga-action']).to eq('Sort by')}
          end
          context 'data-ga-label' do
            it{expect(link.text).to eq(option)}
          end
        end
      end
    end
    ...
    context 'Start Over button' do
      ...
      context 'data-ga-action' do
        it{expect(link.find(:xpath, 'ancestor-or-self::*[@data-ga-action]')['data-ga-action']).to eq('Search Bar Click')}
      end
      context 'data-ga-label' do
        it{expect(link.text).to eq('Start Over')}
      end
    end
    ...
    context 'advanced search toggle' do
      ...
      context 'switch to advanced search' do
        ...
        context 'data-ga-label' do
          it{expect(link.text).to eq('Advanced Search')}
        end
      end
      context 'switch to basic search' do
        ...
        context 'data-ga-label' do
          it{expect(link.text).to eq('Basic Search')}
        end
      end
    end
  end
  ...
  context 'item detail page' do
      ...
      ['Print', 'Email', 'Send to Phone', 'Add to My Saved List', 'Export to EndNote', 'Display in CLIO Legacy',
       ...
       "In-Process / On Order", 'Precataloging', 'Off-site request', 'Item Not On Shelf?', 'Start Over'].each do |option_link|
        context "#{option_link}" do
          ...
          context 'data-ga-category' do
            it{expect(link.find(:xpath, 'ancestor-or-self::*[@data-ga-category]')['data-ga-category']).to eq('Catalog Item Detail')}
          end
          context 'data-ga-action' do
            it{expect(link.find(:xpath, 'ancestor-or-self::*[@data-ga-action]')['data-ga-action']).to eq('Toolbar Click')}
          end
          context 'data-ga-label' do
            it{expect(link.text).to eq(option_link)}
          end
        end
      end
    end
    ...
    context 'catalog item detail' do
      context 'search toggle' do
        ["Advanced", "Basic"].each do |option_link|
          context "Switch to #{option_link}" do
            ...
            context 'data-ga-label' do
              it{expect(link.text).to eq("#{option_link} Search")}
            end
          end
        end
      end
      ...
      context 'Subjects' do
        ...
        context 'data-ga-action' do
          it{expect(link.find(:xpath, 'ancestor-or-self::*[@data-ga-action]')['data-ga-action']).to eq('Subjects Click')}
        end
        context 'data-ga-label' do
          it{expect(link.text).to eq(link.text)}
        end
      end
      ...
      context "Subjects (Genre)" do
        ...
        context 'data-ga-action' do
          it{expect(link.find(:xpath, 'ancestor-or-self::*[@data-ga-action]')['data-ga-action']).to eq('Subjects (Genre) Click')}
        end
        context 'data-ga-label' do
          it{expect(link.text).to eq(link.text)}
        end
      ...
      context 'Author' do
        ...
        context 'data-ga-action' do
          it{expect(link.find(:xpath, 'ancestor-or-self::*[@data-ga-action]')['data-ga-action']).to eq('Author Click')}
        end
        context 'data-ga-label' do
          it{expect(link.text).to eq(link.text)}
        end
      ...
      context 'Also Listed Under' do
        ...
        context 'data-ga-action' do
          it{expect(link.find(:xpath, 'ancestor-or-self::*[@data-ga-action]')['data-ga-action']).to eq('Also Listed Under Click')}
        end
        context 'data-ga-label' do
          it{expect(link.text).to eq(link.text)}
        end
      ...
      context 'Medical Subjects' do
        ...
        context 'data-ga-action' do
          it{expect(link.find(:xpath, 'ancestor-or-self::*[@data-ga-action]')['data-ga-action']).to eq('Medical Subjects Click')}
        end
        context 'data-ga-label' do
          it{expect(link.text).to eq(link.text)}
        end
    ...
    context 'journal item detail' do
      ...
      context 'Subjects' do
        let(:link){page.all(:xpath, "//a[contains(@href, 'subject')]").first}
        context 'data-ga-category' do
          it{expect(link.find(:xpath, 'ancestor-or-self::*[@data-ga-category]')['data-ga-category']).to eq('Journals Item Detail')}
        end
        context 'data-ga-action' do
          it{expect(link.find(:xpath, 'ancestor-or-self::*[@data-ga-action]')['data-ga-action']).to eq('Subjects Click')}
        end
      ...
      context 'Absorbed' do
        ...
        context 'data-ga-action' do
          it{expect(link.find(:xpath, 'ancestor-or-self::*[@data-ga-action]')['data-ga-action']).to eq('Absorbed Click')}
        end
        context 'data-ga-label' do
          it{expect(link.text).to eq(link.text)}
        end
  ...
  context "Academic Commons" do
    ...
    context 'title click' do
      ...
      context 'data-ga-action' do
        it{expect(link.find(:xpath, 'ancestor-or-self::*[@data-ga-action]')['data-ga-action']).to eq('Title Click')}
      end
      context 'data-ga-label' do
        it{expect(link.text).to eq(link.text)}
      end
