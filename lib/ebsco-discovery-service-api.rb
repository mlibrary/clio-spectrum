# 
# LOCAL COPY OF THE SINGLE RUBY FILE FOUND IN
# THE ebsco-discovery-service-api GEM, VERSION 1.0.4.
#
# Copied locally to allo debugging, etc.
# 
require 'net/http'
require 'cgi'
require 'json'
...
module EDSApi
  ...
  class Connection
    ...
    def uid_init(userid, password, profile, guest = 'y')
      ...
      @profile = profile
      @guest = guest
      Rails.logger.debug "Connection#uid_init setting @guest to #{guest}"
      return self
    end
    ...
    # Create the session
    def create_session
      Rails.logger.debug "==== Connection#create_session creating new session with @guest=#{guest}"
...
#      }
  ...
  class ConnectionHandler < Connection
    ...
    def create_session(auth_token = @auth_token, format = :xml)
      Rails.logger.debug "==== ConnectionHandler#create_session creating new session with @auth_token=#{auth_token}"
